...
function init()
{
	...
	//menu.save.addEventListener('click', saveCanvasToLocalStorage, false);
	//menu.save.addEventListener('touchend', saveCanvasToLocalStorage, false);
	//menu.load.addEventListener('click', loadCanvasFromLocalStorage, false);
	//menu.load.addEventListener('touchend', loadCanvasFromLocalStorage, false);
	//menu.exportPic.addEventListener('click', onMenuSave, false);
	//menu.exportPic.addEventListener('touchend', onMenuSave, false);
	...
	container.appendChild(menu.wrapper);
...
function onWindowKeyDown( event )
{
	...
	switch(event.keyCode)
	{
		...
		case 68: // d
			if(BRUSH_SIZE > 1) {BRUSH_SIZE --; menu.brushSize.value=BRUSH_SIZE;}
			break;
...
//{
//	if (BRUSHES[brushSelectorId] == "")return;
//	brush.destroy();
//	brush = eval("new " + BRUSHES[brushSelectorId] + "(context)");
//	window.location.hash = BRUSHES[brushSelectorId];
//}
...
function onMenuClear()
{
	...
	brush.destroy();
	if(menu.selector==null)	brush = eval("new " + BRUSHES[0] + "(context)");
	else brush = eval("new " + BRUSHES[menu.selector.selectedIndex] + "(context)");
}
...
function onMenuAbout()
{
	...
	//about.show();
	//aboutDialogBox();
	//aboutDialogBox("About","About content");
}
...
{
	localStorage.canvas = canvas.toDataURL('image/jpg');
}
...
function loadCanvasFromLocalStorage()
{
	...
	localStorageImage.src = localStorage.getItem("sKey");			
}
